...
const ZONE_DATA = {
  ...
  function activateZone(btn) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (curr.w === prev.h && curr.h === prev.w) {
        curr.isRotated = true;
    ...
    if (btn.img) paperContent += `<img src="${btn.img}" style="width:100%;display:block;">`;
    ...
    function renderZones() {
      ...
      parts.forEach((part) => {
        ...
        if (btn.img) content += `<img src="${btn.img}" class="zi" style="width:100%;display:block;">`;
        ...
        if (part.isRotated) {
          innerContent.style.transform = `translateX(${part.w}px) rotate(90deg) translate(-${part.sX}px, -${part.sY}px)`;
        } else {
          innerContent.style.transform = `translate(-${part.sX}px, -${part.sY}px)`;
        }
  ...
  document.addEventListener('click', () => {
    document.querySelectorAll('.dynamic-zone').forEach(el => el.remove());
  });